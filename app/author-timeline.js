(function (root) {
  ...
  const eras = [
    {id:'ancient', es:'La Antigüedad', en:'Antiquity', from:-800, to:500, min:-800, max:500, ticks:[-800,-400,1,400]},
    {id:'medieval', es:'Los siglos medievales', en:'The medieval centuries', from:500, to:1400, min:500, max:1500, ticks:[500,750,1000,1250,1500]},
    {id:'early-modern', es:'Del Renacimiento al Barroco', en:'From Renaissance to Baroque', from:1400, to:1700, min:1400, max:1800, ticks:[1400,1500,1600,1700,1800]},
    {id:'modern', es:'La modernidad', en:'Modernity', from:1700, to:1900, min:1700, max:2000, ticks:[1700,1800,1900,2000]},
  ...
  function label(era, lang) { return era.from === undefined ? era[lang] : `${era[lang]} · ${year(era.from, lang)}–${year(era.to, lang)}`; }
  // Without a search, the timeline shows one period: the chosen one, or the earliest with authors.
  function currentEra(people, eraFilter) {
    const present = eras.filter(e => people.some(p => eraOf(p) === e.id));
    return (present.find(e => e.id === eraFilter) || present[0] || eras[0]).id;
  }
  ...
  function results(readings,biographies,lang,query='',eraFilter='all') {
    ...
    const only=q||eraFilter==='all'?'all':currentEra(all,eraFilter);
    const filtered=all.filter(p=>(!q||normalise(p.author).includes(q))&&(only==='all'||eraOf(p)===only));
    ...
    if(!filtered.length)return html+`<p class="timeline-empty">${es?'No hay autores que coincidan. Prueba otro nombre.':'No matching authors. Try another name.'}</p>`;
